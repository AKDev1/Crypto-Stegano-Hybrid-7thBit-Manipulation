...
def stego7Bit_img(Ih_, Ip_):
    Ip = cv.cvtColor(Ip_, cv.COLOR_BGR2GRAY)
    p, q = Ip.shape
    # print(img)
    # print(p, q)
    binaryString = ""
    for i in range(0, p):
        for j in range(0, q):
            binaryString = binaryString + format(Ip[i, j], '08b')
    N = len(binaryString)
    print(N)
    
    # I_ = cv.imread(r'C:\\Users\\HP\\Desktop\\ISAA\\J\\assets\\in\\wheel.png')
    Ih = cv.cvtColor(Ih_, cv.COLOR_BGR2GRAY)
    r, c = Ih.shape
    ...
    path = 'C:\\Users\\HP\\Desktop\\ISAA\\J\\Hybrid_Stegano_Crypto\\tempDir\\'
    cv.imwrite(path+'stegoFile_img.png', X)
    st.success("The image was successfully saved in tempDir\stegoFile.png")

def stego7Bit_txt(I_, message):
    N = len(message)*8
    ...
    path = 'C:\\Users\\HP\\Desktop\\ISAA\\J\\Hybrid_Stegano_Crypto\\tempDir\\'
    cv.imwrite(path+'stegoFile_txt.png', X)
    st.success("The image was successfully saved in tempDir/stegoFile.png")

def revStego7Bit_img(img_, p, q):
    ...
    cv.imwrite(path+'stego_7bit_output.png', img_final)
    st.success("The image was successfully saved in tempDir/stego_7bit_output.png")


def revStego7Bit_txt(img_):
    img = cv.cvtColor(img_, cv.COLOR_BGR2GRAY)
    r, c = img.shape
    k = 0
    N = img[0, 0]
    ...
    st.text("The Decoded message is: ")
    st.success(ascii_string)

def main():
    st.title("Hybrid approach to secure data combining Cryptography and Steganography")
    cp = crptoo()
    menu = ["Home", "Steganography-LSB", "Steganography-7th Bit", "Crpytography"]
    submenu = ["Text", "File"]
    ch = ["Encode", "Decode"]
    imgText = ["Text", "Image"] 
    ...
    if choice == "Home":
    ...
    elif choice == "Steganography-7th Bit":
        st.text("Steganography: 7th Bit Manipulation")
        ...
        if x is not None:
            ...
            with open(os.path.join("tempDir", "host_file_7bit_" + x.name), "wb") as f:
                f.write(x.getbuffer())

            host_path = "tempDir/host_file_7bit_" + x.name

            Ih = cv.imread(host_path)
            # print(I)

            cho1 = st.selectbox("What would you like to Encode?", imgText)

            cho = st.selectbox("Operation Type", ch)
            if cho1 == "Text":
                if cho == "Encode":
                    message = st.text_input("Enter the message to be encoded: ")
                    if st.button("Encrypt"):
                        stego7Bit_txt(Ih, message)
            
                else:
                    # msg_len = st.number_input("Enter the message length: ")
                ...
                if cho == "Encode":
                    x1 = st.file_uploader(
                        "Upload the Parasite file", type=["png", "jpg"]
                    )
                    ...
                    if x1 is not None:
                        ...
                        with open(os.path.join("tempDir", "parasite_file_7bit_" + x1.name), "wb") as f:
                            f.write(x1.getbuffer())
                        parasite_path = "tempDir/parasite_file_7bit_" + x1.name

                        Ip = cv.imread(parasite_path)
                    
                    # img = cv.cvtColor(x, cv.COLOR_BGR2GRAY)
                    # message = st.text_input("Enter the message to be encoded: ")
                    if st.button("Encrypt"):
                        stego7Bit_img(Ih, Ip)
                    ...
                    # msg_len = st.number_input("Enter the message length: ")
                    p = st.number_input("enter rows")
                    q = st.number_input("enter columns")
                    if st.button("Decrypt"):
                        # if p != "" and q != "":
                        revStego7Bit_img(Ih, int(p), int(q))


    elif choice == "Steganography-LSB":
        cho = st.selectbox("Operation Type", ch)
        ...
        if choice1 == "HYBRID":
            ...
            if op == "Encode":
                ...
                x = st.number_input("Enter the Number(>=1024) for the generation of Keypair", 1024, 3072)
                if x >= 1024:
                    keyPair = RSA.generate(int(x))
                ...
                # st.text(type(y))
                # keyPair_ = st.text_input("Entr the keyPair you want to Decode")
                y = ast.literal_eval(y)
                # st.text(type(y))
                file2 = open('keyfile.pem', 'rb')
                keyPair = RSA.importKey(file2.read())
                # st.text(keyPair)
                # st.text(type(y))
                # st.text(y)
                if st.button("Decrypt"):
